pipeline {
    ...
    stages {
        stage('AWS') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'my-aws', passwordVariable: 'AWS_SECRET__ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
                    sh '''
                        aws --version
                        aws s3 ls
                    '''
        ...
        stage('staging e2e test') {
            ...
            environment {
                CI_ENVIRONMENT_URL = "${env.STAGING_UTL}"
            }
            steps {
                sh '''
